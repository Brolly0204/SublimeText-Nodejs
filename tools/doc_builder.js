...
var stdLibs = [
  'assert', 'buffer', 'child_process', 'cluster', 'console', 'tls', 'crypto',
...
var saveCompletion = function(options, completions, callback) {
  ...
  fs.writeFile(options.output, txt, (err) => {
    if (err) throw err;
  });
  callback(null, txt)
}
...
var createNamespaces = function(options, files, output) {
  ...
  for(;i<j;i++) {
    ...
    var rKey
    for (rKey in item) {
      if (rKey === 'DEFAULT_ENCODING' || rKey === 'createCredentials' || rKey === 'Credentials') {
        continue;
      }
